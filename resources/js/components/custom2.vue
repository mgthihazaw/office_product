  ...
  mounted() {
    ...
    listElm.addEventListener("scroll", e => {
      ...
      if (
        ...
        this.items[parseInt(page) * 100 + 50] == undefined
      ) {
        ...
        function() {
          ...
          if (this.items[parseInt(page) * 100] == undefined) {
            for (var i = 0; i < 100; i++) {
              ...
              this.showItem[i] = "Item" + (parseInt(page) * 100 + i);
            }
            if (page == 0 || page == 999) {
              this.pTop = parseInt(page) * 100 * 48;
              this.pBottom = this.pTotal - this.pTop;
              this.showItem = this.items.slice(
                parseInt(page) * 100,
                parseInt(page) * 100 + 100
              );
            } else {
              this.pTop = parseInt(page) * 100 * 48 - 480;
              this.pBottom = this.pTotal - this.pTop - 960;
              this.showItem = this.items.slice(
                parseInt(page) * 100 - 10,
                parseInt(page) * 100 + 100 + 10
              );
            }
            // if (this.items[parseInt(page) * 100 - 10] != undefined) {
            //   console.log("TO BOTTOM");
            //   this.pTop = parseInt(page) * 100 * 48 - 480;
            //   this.pBottom = this.pTotal - this.pTop - 480;
            //   this.showItem = this.items.slice(
            //     parseInt(page) * 100 - 10,
            //     parseInt(page) * 100 + 100
            //   );
            // }
            // if (this.items[parseInt(page) * 100 + 110] != undefined) {
            //   console.log("TO BOTTOM");
            //   this.pTop = parseInt(page) * 100 * 48;
            //   this.pBottom = this.pTotal - this.pTop - 480;
            //   this.showItem = this.items.slice(
            ...
            //   );
            // }
            // if (this.items[parseInt(page) * 100 - 10])
            this.loading = false;
            ...
            if (page == 0 || page == 999) {
              if (page == 0) {
                this.pTop = 0;
                this.pBottom = this.pTotal;
              } else {
                this.pTop = this.pTotal;
                this.pBottom = 0;
              }
              //   this.pTop = parseInt(page) * 100 * 48;
              //   this.pBottom = this.pTotal - this.pTop;
