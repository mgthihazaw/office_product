  ...
  <div>
    ...
    >
      <v-toolbar flat color>
        ...
        <v-btn icon color="secondary" @click="multipleDelete">
...
export default {
  ...
  created() {
    axios.get("/api/keys/count").then(res => {
      ...
      this.infiniteInitialRowCount = res.data.count + 1;
    });
  },
  ...
  methods: {
    ...
    multipleDelete() {
      ...
      this.gridApi.getSelectedNodes().forEach(node => {
        console.log(node);
        selectedNodes.push(node.rowIndex);
      });
      console.log("Node", this.gridApi.getSelectedRows());
